...
for k = 1:size(poreims,3)
    [ outbound{k} , inbound{k} ] = genGradBounds(poreims(:,:,k));
...
for k = 1:numel(nout)
    pts1 = outbound{k};
    pts2 = inbound{k};
    pts1 = (tfParams.c*rr*pts1')';
    pts1(:,1) = pts1(:,1) + tfParams.x0;
    pts1(:,2) = pts1(:,2) + tfParams.y0;
    nout{k} = pts1;
    pts2 = (tfParams.c*rr*pts2')';
    pts2(:,1) = pts2(:,1) + tfParams.x0;
    pts2(:,2) = pts2(:,2) + tfParams.y0;
    nin{k} = pts2;
